...
# Cell 5
x_train = np.load("/content/drive/My Drive/bnn/data/svhn/train_data.npy")
y_train = np.load("/content/drive/My Drive/bnn/data/svhn/train_targets.npy")
x_train = x_train / 255.
...
def transfer_weights(model):
    model.transferstate(np.load(prefix + "weights/svhn/svhn_mean.npy", allow_pickle=True))
...
def train(train_images, train_labels, alpha, iterations, batch_size, checkpoint=None, **kwargs):
    ...
    if checkpoint is None:
        train_config = {'learning_rates': [alpha], 'batch_sizes': [batch_size], 
                        ...
                        'total_losses': []}
        weight_file = "weights/svhn/" + curr_date_time
...
def test(test_images, test_labels, samples, weight_file, batch_size=64, **kwargs):
...
# Cell 8
train(x_train, y_train, 0.01, 50, 128, checkpoint="weights/svhn/2020_10_03_12_25_37_333954", c=1, b=1e6, a=1)

# Cell 9
test(x_test, y_test, 1, "weights/svhn/2020_10_03_12_25_37_333954")
